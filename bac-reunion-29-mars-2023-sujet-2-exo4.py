...
##################################################
# Réunion 29 mars 2023 Sujet 2
##################################################

# Exo 4 Question 1
class ReunionExo4Question1(Scene):
    def construct(self):
        msg1 = "Bac Reunion 28 mars 2023 Sujet 2 Exercice 4"
        title1 = Title(f"{msg1}")
        ...
        q1_txt = [
            r"1. On considère la fonction \(f\) définie sur \(\mathbb{R}\) par",
            r"\(f(x) = 2xe^x\).",
            r"Le nombre de solutions sur \(\mathbb{R}\) de l'équation \(f(x) = -\dfrac{73}{100}\) est égal à : "
        ]
        ...
        C00 = r"La fonction \(f\) est dérivable comme produit de fonctions dérivables."
        C01 = r"Ainsi en posant \(u = 2x\) et \(v = e^x\) à l'aide de la formule \((uv)' = u'v + uv'\)"
        C02 = r"On obtient \(f'(x) = 2e^x + 2xe^x = 2(x + 1)e^x\)"
        C03 = r"Ainsi \(f\) est décroissante sur l'intervalle \(]-\infty; -1]\)"
        C04 = r"Puis, \(f\) est croissante sur l'intervalle \([-1; +\infty[\)"
        C05 = r"Or \(f(-1) = -2e^{-1} \simeq -0,74 < -\dfrac{73}{100}\)"
        C06 = r"D'après le théorème des valeurs intermédiaires"
        C07 = r"et la monotonie de \(f\) sur chaque intervalle"
        C08 = r"il existe exactement une solution sur chacun soit 2 sur \(\mathbb{R}\)"
        
        C = [C00, C01, C02, C03, C04, C05, C06, C07, C08]
        dC = [Tex(d).scale(0.85) for d in C]
...
class FollowingGraphCamera(MovingCameraScene):
    def construct(self):
        ...
        ax = Axes(x_range=[-3.5, 1.5, 0.25], y_range=[-1, 14, 0.25])
        graph = ax.plot(
            lambda x: 2*x*np.exp(x),
            color=BLUE,
            x_range=[-3.5, 1.5]
        )
        ...
        # create dots based on the graph
        moving_dot_level = Dot(ax.i2gp(graph_level.t_min, graph), color=YELLOW)
        dot_level_1 = Dot(ax.i2gp(graph_level.t_min, graph_level))
        dot_level_2 = Dot(ax.i2gp(graph_level.t_max, graph_level))
        ...
        self.play(
            self.camera.frame.animate.scale(0.5).move_to(moving_dot_level)
        )
        ...
        # # Creating the axes and function graph
        # axes = Axes(
        #     x_range=[-1.5, 0.5, 0.25],
        #     y_range=[-1, 0.25, 0.05],
        #     x_length=8,
        #     y_length=8,
        #     tips=True,
        ...
        # func = lambda x: 2 * x * np.exp(x)
        # level_line = lambda x: -0.73
        # tangente_line = lambda x: -0.74
        ...
        #     x_range=[-1.5, -0.5, 0.25]
        # )
        # graph_level = axes.plot(
        #     level_line,
        #     color=GREEN,
        #     x_range=[-1.5, -0.5, 0.25]
        # )
        # graph_tangente = axes.plot(
        #     tangente_line,
        #     color=RED,
        #     x_range=[-1.5, -0.5, 0.25]
        # )
        # graphs = [graph, graph_level, graph_tangente]
        
        # # Displaying the axes and function graph
        # self.play(
        #     ReplacementTransform(q1[-1], axes),
        #     FadeOut(dC[-1]),
        #     *[Write(o) for o in graphs]
        # )
        # self.wait(2)

# Question 2
class ReunionExo4Question2(Scene):
    def construct(self):
        msg1 = "Bac Reunion 28 mars 2023 Sujet 2 Exercice 4"
        title2 = Title(f"{msg1}")
...
# Question 3
class ReunionExo4Question3(Scene):
    def construct(self):
        msg1 = "Bac Reunion 28 mars 2023 Sujet 2 Exercice 4"
        title3 = Title(f"{msg1}")
...
# Question 4
class ReunionExo4Question4(Scene):
    def construct(self):
        msg1 = "Bac Reunion 28 mars 2023 Sujet 2 Exercice 4"
        title4 = Title(f"{msg1}")
...
# Question 5
class ReunionExo4Question5(Scene):
    def construct(self):
        msg1 = "Bac Reunion 28 mars 2023 Sujet 2 Exercice 4"
        title5 = Title(f"{msg1}")
